...
return (
    ...
    <Link href={`/products/${product.id}`} className="flex items-center space-x-4 group">
     ...
     <div className="flex items-center">
        ...
        <button onClick={() => setItemQuantity(product.id,product.quantity - 1)} disabled={product.quantity <= 1}  className="border rounded-full w-8 h-8 flex items-center justify-center hover:bg-rose-100
         hover:text-rose-500">-</button>
        <p className="font-semibold text-xl">{product.quantity}</p>
        <button onClick={() => setItemQuantity(product.id,product.quantity + 1)} className="border rounded-full w-8 h-8 flex items-center justify-center  hover:bg-green-100
         hover:text-green-500">+</button>    
        </div>
